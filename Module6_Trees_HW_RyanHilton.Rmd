...
```
I think we might have problems with predicting yes's. When there is such an unbalanced number of one type of classification, we will always be very bad at predicting the less frequent classification. In the case of decision trees, if the less frequent classification isn't clumped together in the data space to make it somewhat easy to create a pocket in the data space, our decision trees will be very poor because the small pockets of the data space in our decision trees based off the training data are very unlikely to have the less frequent classification in those pockets.
...
```
This is what I was concerned about. Our decision tree classifies everything as No. This actually makes a lot of sense. We saw our logistic regressions had more incorrect classifications of yes than correct classifications of yes. When this occurs, predicting everything as No will in fact produce a higher accuracy than attempting to predict something as Yes. Given our data, it is so difficult to predict yes because its either hidden with a bunch of No's or the distribution of Yes's is so scattered that it's impossible to narrow in on a set data space where there will be correct classifications of Yes. So this decision tree actually produces the highest accuracy of any model we can find, even though every terminal node is classified as a No.
...
```{r}
plot(tree.caravan.VIF)
text(tree.caravan.VIF, pretty=0)
```
...
rownames(c1.table) = c("Full Logistic Regression","Reduced Logistic Regression")
...
rownames(c1.table2) = c("Full Logistic Regression","Reduced Logistic Regression")
...
rownames(d1.table) = c("Full Logistic Regression","Reduced Logistic Regression")
...
rownames(d1.table2) = c("Full Logistic Regression","Reduced Logistic Regression")
...
rownames(e1.table) = c("Full Logistic Regression","Reduced Logistic Regression")
...
rownames(e1.table2) = c("Full Logistic Regression","Reduced Logistic Regression")
...
```{r}
best.iter = gbm.perf(boost.caravan, method = "OOB")
```

```{r}
boost.probs.tr = predict.gbm(boost.caravan, newdata=Caravan.train, n.trees = best.iter, type = "response")
boost.probs.tr = as.matrix(boost.probs.tr)
boost.pred.tr <- ifelse(boost.probs.tr > 0.5, 1, 0)
...
boost.probs.ts = predict.gbm(boost.caravan, newdata=Caravan.test, n.trees = best.iter, type = "response")
boost.probs.ts = as.matrix(boost.probs.ts)
boost.pred.ts <- ifelse(boost.probs.ts > 0.5, 1, 0)
